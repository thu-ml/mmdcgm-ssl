'''
This code implements max-margin conditional deep generative model which incorporates the side information in generative modelling and uses a discriminative classifier to infer the latent labels
...
if dataset == 'mnist_real':
    ...
    train_x, train_t, valid_x, valid_t, test_x, test_t = load_svhn(normalized=True, centered=False)
    if flag == 'validation':
        test_x = valid_x
        test_t = valid_t
    else:
...
preprocesses_dataset = lambda dataset: dataset
sh_x_train = theano.shared(preprocesses_dataset(train_x), borrow=True)
sh_t_train = theano.shared(train_t, borrow=True)
...
for i in (xrange(nlayers_dec-1)):
    ...
    l = decodelayer(l_dec[-1],up_method[i],False,dr_dec[i],ps_dec[i],nk_dec[i],dk_dec[i],nonlin_dec[i],pad_dec[i],str_dec[i],'DEC-'+str(i+1))
    l_dec.append(l)
    print lasagne.layers.get_output_shape(l_dec[-1])

# mu and var
if distribution == 'gaussian':
    l_dec_x_mu = decodelayer(l_dec[-1],up_method[-1],False,dr_dec[-1],ps_dec[-1],nk_dec[-1],dk_dec[-1],lasagne.nonlinearities.identity,pad_dec[-1],str_dec[-1],'DEC-MU')
    l_dec_x_log_var = decodelayer(l_dec[-1],up_method[-1],False,dr_dec[-1],ps_dec[-1],nk_dec[-1],dk_dec[-1],lasagne.nonlinearities.identity,pad_dec[-1],str_dec[-1],'DEC-LOG_VAR')
elif distribution == 'bernoulli':
    l_dec_x_mu = decodelayer(l_dec[-1],up_method[-1],False,dr_dec[-1],ps_dec[-1],nk_dec[-1],dk_dec[-1],lasagne.nonlinearities.sigmoid,pad_dec[-1],str_dec[-1],'DEC-MU')
...
accurracy_train = lasagne.objectives.categorical_accuracy(predictions_train, sym_y)
...
classifier_cost_train = multiclass_hinge_loss(predictions=predictions_train, targets=sym_y, weight_decay=weight_decay_classifier, alpha_decay=alpha_decay)
classifier_cost_eval = multiclass_hinge_loss(predictions=predictions_eval, targets=sym_y, weight_decay=weight_decay_classifier, alpha_decay=alpha_decay)
...
if distribution == 'bernoulli':
    z_train, z_mu_train, z_log_var_train, x_mu_train = lasagne.layers.get_output([l_z, l_mu, l_log_var, l_dec_x_mu], {l_in_x:sym_x,l_in_y:predictions_train_hard}, deterministic=False)
    ...
    z_train, z_mu_train, z_log_var_train, x_mu_train, x_log_var_train = lasagne.layers.get_output([l_z, l_mu, l_log_var, l_dec_x_mu, l_dec_x_log_var], {l_in_x:sym_x, l_in_y:predictions_train_hard}, deterministic=False)
...
ran_z = T.tile(srng_ran.normal((10,nz)), (10, 1))
if distribution == 'bernoulli':
...
def train_epoch(lr, eq_samples, iw_samples, batch_size):
    ...
    for i in range(n_train_batches):
        costs_batch, log_qz_given_xy_batch,log_pz_batch,log_px_given_zy_batch, loss_batch, accurracy_batch = train_model(i, batch_size, lr, eq_samples, iw_samples)
        costs += [costs_batch]
...
# TRAIN LOOP
LL_train, log_qz_given_x_train, log_pz_train, log_px_given_z_train, loss_train, acc_train = [],[],[],[],[],[]
...
for epoch in range(1, 1+num_epochs):
    ...
    sh_x_train.set_value(preprocesses_dataset(train_x[p]))
    sh_t_train.set_value(train_t[p])
